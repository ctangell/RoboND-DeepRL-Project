...
#define NUM_ACTIONS 2*DOF // Not sure what this value should be, this is just a guess.
...
#define OPTIMIZER "RMSprop"  // "None" produces error
...
void ArmPlugin::Load(physics::ModelPtr _parent, sdf::ElementPtr /*_sdf*/) 
{
	...
	*/
	...
	//cameraSub = None;
	cameraSub = cameraNode->Subscribe("/gazebo/arm_world/camera/link/camera/image", &ArmPlugin::onCameraMsg, this); // class_instance not strictly necessary
...
bool ArmPlugin::updateAgent()
{
	...
	*/
    ...
    if (action % 2 == 0) velocity += actionVelDelta;
...
void ArmPlugin::OnUpdate(const common::UpdateInfo& updateInfo)
{
	...
	if( hadNewState && !newReward )
	{
		...
		/
		*/
		bool checkGroundContact = gripBBox.max.z < 0 || gripBBox.min.z < 0; // QUESTION: x or z here?
		
		if(checkGroundContact)
		{
						
			if(DEBUG){printf("GROUND CONTACT, EOE\n");} // does EOE mean "end of episode"?

			rewardHistory = REWARD_LOSS;
			newReward     = true;
			endEpisode    = true; // or true
		}
		...
		*/ 
		...
		if(!checkGroundContact)
		{
			...
			if( episodeFrames > 1 )
			{
				...
				const float alpha = 0.0;  // hyper-parameter to tune
	...
	if( newReward && agent != NULL )
	{
		...
		if( endEpisode )
		{
			...
			totalRuns++;
            totalRuns_sub++;
			printf("Current Accuracy:  %0.4f (%03u of %03u; %03u of %03u)  (reward=%+0.2f %s)\n", float(successfulGrabs_sub)/float(totalRuns_sub), successfulGrabs_sub, totalRuns_sub, successfulGrabs, totalRuns, rewardHistory, (rewardHistory >= REWARD_WIN ? "WIN" : "LOSS"));

          	if (totalRuns_sub >= 100) {
              totalRuns_sub = 0;
              successfulGrabs_sub = 0;
            }
